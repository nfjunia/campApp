  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          variant="default"
          className="w-full bg-white border-neutral-300 hover:bg-white cursor-pointer text-black border h-12 shadow-0 justify-between"
          aria-label={label}
        >
          {selected ? (
      ...
      <DialogContent className="w-full pt-20 h-screen flex flex-col p-0">
          ...
          <ScrollArea className="h-full">
            <div className="space-y-2">
              {filteredCountries.map((country) => (
                <button
                  key={country.code}
                  className="w-full text-left p-2 rounded flex items-center gap-3 hover:bg-gray-100"
                  onClick={() => {
